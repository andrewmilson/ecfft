...
use ark_ff::Field;
use ecfft::ec::GeneralWeierstrassCurve;
use ecfft::ec::Isogeny;
use ecfft::ec::Point;
use ecfft::fftree::RationalMap;
// use ecfft::m31::Fp;
...
fn main() {
    rayon::scope(|s| {
        for _ in 0..10 {
            s.spawn(|_| find_curve::<Fp>());
        }
    });
}

fn is_odd<F: Field>() -> bool {
...
impl<F: Field> GoodCurve<F> {
    ...
    pub fn good_point(self) -> Point<F> {
        match self {
            Self::Even { b } => Point::new(b, b, self.into()),
            Self::Odd { a, b } => Point::new(a, b.square(), self.into()),
        }
    }

    pub fn good_isogeny(self) -> Isogeny<F> {
        match self {
            GoodCurve::Even { b } => {
                let domain = self.into();
                let codomain = Self::new_even(b).into();
            ...
            GoodCurve::Odd { a, b } => {
                ...
                let codomain = Self::new_odd(a_prime, b_prime).into();

                let one = F::one();
                let zero = F::zero();
                let r = RationalMap::new(&[bb, -b.double(), one, one], &[zero, one]);
...
impl<F: Field> From<GoodCurve<F>> for GeneralWeierstrassCurve<F> {
    fn from(val: GoodCurve<F>) -> Self {
        use GoodCurve::*;
        let zero = F::zero();
        match val {
            Odd { a, b } => GeneralWeierstrassCurve::new(zero, a, zero, b.square(), zero),
            Even { b } => GeneralWeierstrassCurve::new(F::one(), zero, zero, b.square(), zero),
        }
    }
}
...
fn find_two_sylow_subgroup<F: Field>(a: F, bb: F) -> (u32, u32, Option<F>, Option<F>) {
    assert!(is_odd::<F>());
    ...
    if discriminant.sqrt().is_some() {
        ...
        if !p4_cond1 && !p4_cond2 && !p4_cond3 {
            ...
            return (1, 1, Some(F::zero()), Some(r1));
        }
...
/// subgroups for ECFFT.
fn find_cyclic_two_sylow_subgroup<F: Field>(a: F, bb: F) -> (u32, Option<F>) {
    assert!(is_odd::<F>());
    let discriminant = a.square() - bb.double().double();
    assert!(!discriminant.is_zero());
...
fn find_curve<F: Field>() {
    if is_odd::<F>() {
        ...
        loop {
            ...
            let (n, x) = find_cyclic_two_sylow_subgroup(a, bb);
            if n > max_n {
                println!("n={n} YO_{a}_{bb}: {:?}", x);
                max_n = n;
            }
        }
        // loop {
        //     // note we are dealing with the model y^2 = x(x^2 + a*x + B), B =
        // b^2     let a = F::rand(&mut rng);
        //     let bb = F::rand(&mut rng);
        //     let discriminant = a.square() - bb.double().double();
        ...
        //     if let Some(b) = discriminant.sqrt() {
        //     } else {
        //     }
        // }
    } else {
